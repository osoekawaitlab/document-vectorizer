...
def test_vectorize(mock_vectorizer: MagicMock) -> None:
    sut = DocumentVectorizerCore(document_vectorizer=mock_vectorizer)
...
def test_batch_vectorize_with_supported_document_vectorizer(mock_vectorizer_supports_batch: MagicMock) -> None:
    sut = DocumentVectorizerCore(document_vectorizer=mock_vectorizer_supports_batch)
...
def test_create(mocker: MagicMock, mock_vectorizer: MagicMock) -> None:
    ...
    settings = DocumentVectorizerCoreSettings(document_vectorizer_settings=AllMiniLmDocumentVectorizerModelSettings())
    actual = DocumentVectorizerCore.create(settings=settings)
